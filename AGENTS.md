...
- ASGI 2.0 apps are a separate type rather than an auto-detected convention. MoonBit has no signature reflection, so there is no equivalent of asgiref's `guarantee_single_callable`; the choice is an explicit constructor and should stay one.
